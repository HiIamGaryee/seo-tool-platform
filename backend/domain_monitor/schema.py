...
SEO_COLUMNS: list[tuple[str, str]] = [
    ...
    ("notes", "TEXT"),
    # Per-source refresh stamps: these sources genuinely refresh at different
    # cadences, so they each get their own timestamp.
...
-- Version 3: many-to-many domain <-> discovery source attribution. A domain
-- found by several sources keeps one row in `domains` and one row here per
-- source, so provenance survives without duplicating candidates.
